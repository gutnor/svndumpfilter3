...
[2012-02-21] A few fixs  from "Bernhard M. Wiedemann" <bwiedemann dot
    ...
    def propparse(self):
        ...
        while 1:
            if self.prop[index:index + 2] == "K ":
                ...
                raise Exception("Unrecognised record in props section")
...
def read_lump(f):
    ...
    lump.orig_text = "".join(lines) + lump.prop + lump.text
...
def fetch_rev_rename(repos, srcrev, srcpath, path, fout, flog, format_dump):
    ...
    if fmtequiv[format_dump] != fmtequiv[format_sub] and not format_warning:
    ...
    assert 'Revision-number' in lump_sub.hdrdict
    ...
    while 1:
        ...
        assert not 'Revision-number' in lump_sub.hdrdict
...
def main():
    ...
    format_version, uuid, text = read_dump_header(fr)
    fw.write(text)

    if format_version not in __supported_versions__:
